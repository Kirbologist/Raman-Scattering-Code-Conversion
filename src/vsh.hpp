...
i.e. mostly low-level functions handling the calculations of quantites related to vector spherical wavegunctions.
*/
...
namespace Smarties {
  ...
  unique_ptr<stPinmTaunm<Real>> vshPinmTaunm(int N_max, const ArrayXr<Real>& theta) {
    ...
    ArrayXr<Real> mu_c = cos(theta);
    ArrayXr<Real> mu_s = sin(theta);
    ...
    ArrayXXr<Real> p_nm1(n_rows, N_max + 1);
    ArrayXXr<Real> t_nm1(n_rows, N_max + 1);
    p_nm1.col(0).setOnes();
    p_nm1.col(1) = mu_c;
    t_nm1.col(0).setZero();
    t_nm1.col(1) = -mu_s;
  ...
  unique_ptr<stIncEabnm<Real>> vshGetIncidentCoeffs(int N_max, const unique_ptr<stIncPar<Real>>& angles) {
    ...
    for (int n = 0; n <= N_max; n++) {
      ArrayXi m = ArrayXi::LinSpaced(2*n + 1, -n, n);
      ArrayXi ind = n*(n + 1) + m;
  ...
  unique_ptr<stEAllPhi<Real>> vshEgenThetaAllPhi(
    ...
    for (int m = -N_max; m <= N_max; m++) {
      ArrayXi n_vec = ArrayXi::LinSpaced(N_max - abs(m) + 1, abs(m), N_max);
      ArrayXi p_vec = n_vec*(n_vec + 1) + m;
      ArrayXXr<Real> pi_nm = stPT->pi_nm(all, p_vec);
      ArrayXXr<Real> tau_nm = stPT->tau_nm(all, p_vec);
      ArrayXXr<Real> d_nm = m ? pi_nm.colwise()*(sin(theta)/m).transpose() : stPT->p_n0;
      ...
      for (int l = 0; l < Nb_lambda; l++) {
        ...
        VectorXc<Real> vec_n_dep = (q_nm_for_Z1.row(l) * mu_n_times(n_vec).transpose()).matrix();
        Er_sum.row(l) = ((d_nm*st_zn_all_col->Z1(ind_in_rho_col, n_vec)).matrix() * vec_n_dep).transpose().array();
        vec_n_dep = (ip_nm_for_Z0.row(l) * mu_n_divd(n_vec).transpose()).transpose().matrix();
        VectorXc<Real> vec_n_dep2 = (q_nm_for_Z2.row(l) * mu_n_divd(n_vec).transpose()).transpose().matrix();
